...
export class Password {
	...
	private static validatePassword(password: string): string[] {
		const errors: string[] = [];
		if (!this.hasSixCharactersOrMore(password)) errors.push('Password is too short');
		if (!this.containsNumber(password)) errors.push('Password must contain a number');
		if (!this.containsLowerCase(password)) errors.push('Password must contain a lowercase letter');
		if (!this.containsUpperCase(password)) errors.push('Password must contain an uppercase letter');
		if (!this.containsUnderscore(password)) errors.push('Password must contain an underscore');
		return errors;
	}
